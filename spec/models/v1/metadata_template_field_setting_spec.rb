...
describe V1::MetadataTemplateFieldSetting do
  ...
  let(:template_field_setting) do
    attrs = { :metadata_field_id => field.id, :metadata_template_id => template.id }
  ...
  describe "requires a field id" do
    before { template_field_setting.metadata_field_id = nil }
    it { 
      should_not be_valid
      should have(1).error_on(:metadata_field_id)
    }
